# Hornbill
An Offline-First Emergency Response & Navigation Ecosystem
---
Description:
The Hornbill App introduces an offline-first emergency response ecosystem designed to bridge the communication gap prevalent in rural ASEAN region. Using peer-to-peer networking and built-in AI models, Hornbill is able to close the gap on emergency rescue times and improve disaster resilience in rural regions.

---
Team : Last Minute
Members:
1. Lee Jarrell
2. Loo Jiun Wei
3. Janisa Goh Shi Jie
4. Angel Tan Ke Qin

Figma Prototype Link : [https://www.figma.com/proto/5RHyPpOenuU9ldPDU1mZzg/LastMinte-HornBill?node-id=15-492&t=NSCK1TIrDHz96Pfb-1](https://www.figma.com/proto/5RHyPpOenuU9ldPDU1mZzg/LastMinte-HornBill?node-id=15-492&t=NSCK1TIrDHz96Pfb-1)

Youtube Demo Link : [https://youtu.be/d5TdM9fhb40](https://youtu.be/d5TdM9fhb40)
